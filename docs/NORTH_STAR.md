...
| 18 | Offline private inference | 🟡 **artifact ready, demo not run** | The Q8_0 GGUF runs fully local through llama.cpp with no network path (row 2), so the capability is demonstrated. What remains is the airplane-mode *demo* — Wi-Fi off, messy ticket in, redacted text out, wall-clock on screen — plus the G6 OOD probe set, which was not re-measured against the shipped artifact. WP-6. |
