...
run_003 stopped at step 100. `checkpoint-100` was retained as the evidence for this
entry until 2026-09-04, when it was **deleted to reclaim 857 MB**. The evidence is the
measurement, not the weights: the loss curve, F1 0.5379, precision 0.8583, recall 0.3916
and span ratio 0.46 are all recorded above, and the run reproduces from committed code —
`scripts/run_train.py --lora-r 64 --lora-target-mlp --train-data data/train_v2.jsonl`,
seed 42. Noted here rather than left as a dangling reference, because an ADR that points
at an artifact which no longer exists is worse than one that says why it does not.
